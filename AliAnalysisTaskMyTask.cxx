...
AliAnalysisTaskMyTask::AliAnalysisTaskMyTask() : AliAnalysisTaskSE(), 
	fAOD(0), fOutputList(0), fHistPt(0), fHistXv(0), fHistYv(0) ,fHistZv(0), fHistEPhi(0)
{
...
void AliAnalysisTaskMyTask::UserCreateOutputObjects()
{
	...
	fHistXv = new TH1F("fHistXv", "fHistXv", 100, -2, 2);
	fHistYv = new TH1F("fHistYv", "fHistYv", 100, -2, 2);
	fHistZv = new TH1F("fHistZv", "fHistZv", 100, -10, 10);
	fHistEPhi = new TH2F("fHistEPhi", "fHist Eta-Phi", 100, 0, 2.0*TMath::Pi(),100, -2.1, 2.1);

	fOutputList->Add(fHistPt);          // don't forget to add it to the list! the list will be written to file, so if you want
	fOutputList->Add(fHistXv);
	fOutputList->Add(fHistYv);
	fOutputList->Add(fHistZv);
	fOutputList->Add(fHistEPhi);
...
void AliAnalysisTaskMyTask::UserExec(Option_t *)
{
	...
	fHistXv->Fill(fAOD->GetPrimaryVertex()->GetX());
	fHistYv->Fill(fAOD->GetPrimaryVertex()->GetY());
	fHistZv->Fill(fAOD->GetPrimaryVertex()->GetZ());
	...
	//	printf("------------ pt = %f\n",track->Pt() );
		fHistEPhi->Fill(track->Phi(),track->Eta());
